...
def t_fit_transformers(df, func=clean_texts, df_columns=['name']):
    ...
    inputs = tokenizer(df_tmp.tolist(), padding=True, truncation=True,
                       max_length=63, return_tensors='pt')
    with torch.no_grad():
...
# производителя
def t_predict_transformers(dealer_names, product_embedding):
    # Загрузка токенизатора и модели LaBSE_ru_en
    tokenizer = AutoTokenizer.from_pretrained("cointegrated/LaBSE-en-ru")
    model = AutoModel.from_pretrained("cointegrated/LaBSE-en-ru")
    ...
    dealer_embedding = model(**tokenizer(
        ...
        truncation=True, max_length=63, return_tensors='pt'
...
def prosept_predict(product, dealerprice) -> list:
    ...
    # Обучение модели
    product_embedding_transformers = t_fit_transformers(df_product,
                                                        clean_texts, ['name'])
